...
export const filterRouter = (array, access, routers) => {
  let list = array.map(item => {
      ...
      item.menuName = item.fullName
      item.menuCode = item.menuId
    ...
    // if (item.parentId === 0 || item.parentId === '0') {
      if (item.parentId === '60914034d768af34cedfa8d7f0defca2') {
        // 根节点
        router.path = '/'
...
export const listConvertTree = (array, opt) => {
  let obj = {
    ...
    childKey: opt.childKey || 'childMenus'
  }
  return listToTree(array, obj.startPid, obj)
}
...
export const listToTree = (array, startPid, opt, isRoute) => {
  ...
  if (array && array.length > 0) {
    child = array.map(item => {
      ...
      if (typeof item[opt.parentKey] !== 'undefined' && item[opt.parentKey] === startPid) {
        ...
        // let nextChild = listToTree(array, item[opt.primaryKey], opt)
        let nextChild
        //TODO  遍历生成路由为子级
        if (isRoute) {
          nextChild = listToTree(item.childMenus, item[opt.primaryKey], opt, isRoute)
        } else {
          nextChild = listToTree(array, item[opt.primaryKey], opt, isRoute)
        }
        // 节点信息保存
